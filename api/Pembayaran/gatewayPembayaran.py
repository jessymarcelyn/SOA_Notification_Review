...
class GatewayPaymentService:
    ...
    def get_pemabayaran_by_id_pemesanan(self, request, id_pesanan):
        pembayaran = self.payment_rpc.get_payment_by_id_pesanan(id_pesanan)
        return json.dumps(pembayaran)
    
    @http('GET', '/pembayaran/<int:id_pembayaran>')
    def get_pemabayaran_by_id_pemesanan(self, request, id_pembayaran):
        pembayaran = self.payment_rpc.get_payment_by_id_pembayara (id_pembayaran)
        return json.dumps(pembayaran)
    
    @http('GET', '/pembayaran/<int:id_pesanan>')
    def get_pemabayaran_by_id_user(self, request, id_user):
        pembayaran = self.payment_rpc.get_payment_by_id_user(id_user)
        return json.dumps(pembayaran)
